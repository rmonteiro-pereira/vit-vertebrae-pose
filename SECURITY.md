...
the aggregate metrics are publishable. The images are not — the licence forbids
re-posting, and the frames carry burned-in acquisition metadata (study date, time,
technical parameters) that constitutes an indirect identifier when attached to
identifiable anatomy.

**No radiograph, no image derived from a radiograph, and no patient identifier is in
this repository — in the working tree or in its history.**
